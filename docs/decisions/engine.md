...
- **Blast radius:** the `ces_revisions` package, and every roadmap stage that marginalizes the linear-Gaussian states: Stages 6–11, 15–21, 26, and 27, and Stage 22's change-point model
...
| dynamax (with tfp-nightly 0.26.0.dev20260912) | 1.0.2 | dev | Imports with three warnings: `jax.core.pytype_aval_mappings` is deprecated (via tfp-nightly), `parallel_inference.py` has an invalid escape sequence (a `SyntaxWarning`, shown only when that file is compiled rather than loaded from cached bytecode), and `asyncio.iscoroutinefunction` is slated for removal in Python 3.16 (via fastcore) |
...
- **Positive:**
  - One masking path handles NaN cells, fully missing steps, and irregular rows, and the log density is exact over the observed cells. A missing cell's observation-covariance entries reach only `one_step_cov`, even when they are non-finite. The filter and smoother match the dense oracle in `tests/test_kalman.py` at `rtol=1e-8` and `atol=1e-10`; the largest absolute difference observed at execution was 1.1e-13. The log-likelihood gradient matches a finite difference along a joint scaling of `Q_t` and `R_t`, with NaN cells present.
  ...
  - The synthetic pilot in `tests/test_synthetic_pilot.py` covers time-varying emission rows, NaN cells, a ragged real-time edge, a missing month, and a March-only benchmark row. It samples six static scales with four chains, and its slow test asserts Req 18's thresholds: zero divergences, R-hat below 1.01, bulk and tail ESS above 400, energy-BFMI above 0.3 in every chain, under 1% of draws at the maximum tree depth, and every true scale within four posterior SDs. In the recorded run, R-hat was at most 1.0011, bulk ESS at least 5001, and tail ESS at least 2580; energy-BFMI ran from 0.96 to 1.16, no draw used more than 15 leapfrog steps, and every true scale was within 1.6 posterior SDs.
...
- **Negative:**
  ...
  - The filter is dense. Each step costs `O(n^3 + p^3)`: masking keeps all `p` cells, so every step factors a `p × p` innovation covariance however many cells are missing. The outputs hold full `(T, n, n)` and `(T, p, p)` covariance stacks. Both grow expensive as the full model approaches roughly 150 states over several hundred months.
...
- **Dynamax with pre-masked inputs.** This option would zero the observation rows of missing cells in a time-varying `H_t`, give those cells a unit diagonal in `R_t` and zero their covariances with observed cells, zero-fill `y`, and add back `0.5 log(2π)` for each masked cell. Dynamax accepts the `(T, ·, ·)` arrays this needs, so the scalar log likelihood and the smoother are within reach. Per-step contributions and one-step-ahead moments would need a second, vectorized pass that rebuilds the predicted moments from Dynamax's filtered moments and re-applies the masking. The option also adds a hand-maintained constant correction, and puts tfp-nightly and a fastcore web-server dependency chain into the runtime environment. It was not built.
