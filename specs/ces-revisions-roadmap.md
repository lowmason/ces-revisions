...
Resumed 2026-09-15 at `1a57aad`: Stage 3 complete per its stamp, and Stages 4–27 re-validated
against what it shipped. Stage 4 now owns dates for publications outside the Employment Situation
release index; Stages 6 and 8 absorb Stage 3's deferred consumer checks at their first use.
Resumed 2026-09-15 at `b8d65c9`: Stage 4 complete per its stamp, and Stages 5–27 re-validated
against what it shipped. The separate QCEW revision sequence stays in jobs while its precision
proxy is in thousands; actual product-specific publication dates flow through `observable_at`.
Plan 7 hardens Stage 3 acquisition only and changes no roadmap boundary or later-stage interface.
...
|---|---|---|---|
| 1 Vintage panel | missing | none found; `pyproject.toml:9` `dependencies = []` | 2026-09-15: Stage 3 (plan 5) shipped the content-hashed source archive, raw/transformation/long tables, and same-release differencing under `src/ces_revisions/vintages/`; Stage 4 (plan 6) shipped the final/preliminary benchmark anchors and sector contributions under `src/ces_revisions/annual/` |
| 2 Stage definitions | missing | none found | 2026-09-15: Stage 3 (plan 5) shipped F/S/T/B/M labels, right-censored M, and the 2003-gap/2025-lapse/frontier flags in `src/ces_revisions/vintages/stages.py` |
| 3 Covariate panel | missing | none found | FY2009+ in v1; FY1979 is Stage 27. 2026-09-14: Stage 2 (plan 2) inventoried every Req 3 series with its first-publication lag ([data inventory](../docs/ces-revisions-review.md#data-availability-inventory)); the spec's amendment of that date revised the collection-window and sample rows, against which Stages 3, 4, and 12 are re-validated. 2026-09-15: Stage 4 (plan 6) shipped the annual Table 1 coverage proxy and RSE panel, including the March 2012 archive gap and the RSE-definition break |
| 4 Estimation window | missing | none found | 2026-09-15: Stage 3 (plan 5) shipped the May-2003+ sector panel and the partially observed 1979+ aggregate leg; model use remains in Stages 6–9 |
| 5 Sector set / aggregation | missing | none found | 2026-09-15: Stage 3 (plan 5) shipped and tested the eleven-supersector/total data reconciliation; the model and operator halves remain in Stages 5 and 7 |
...
| 9 Joint NSA/SA + decomposition | missing | none found | channel (b) is Stages 25–26; carries an (open). 2026-09-14: Stage 2 (plan 2) discharged the (open) archive item (bullet 4) in `docs/inventory/archive-inventory.csv`: all three file types survive for 28 of 279 releases and no release has unrounded NSA inputs, so plan 2's parking condition (no release keeps all three) did not fire and Stages 25–26 stay live, with Stage 25 reproducing from rounded inputs ([consequences](../docs/ces-revisions-review.md#consequences-for-later-stages)). 2026-09-15: Stage 3 (plan 5) shipped the published-panel accounting identity and variance decomposition in `src/ces_revisions/vintages/accounting.py`; the model-based halves remain in Stages 9 and 26 |
| 10 Benchmark observations + operators | missing | none found | carries an (open). 2026-09-15: Stage 4 (plan 6) shipped dated preliminary/final anchors, reconstruction events, and separate QCEW revision/precision inputs; Stages 5 and 8 retain the operator determination and model halves |
| 11 Birth–death | missing | none found | 2026-09-15: Stage 4 (plan 6) shipped NSA monthly forecast/revised schedules and annual forecast-versus-realized rows, with government structurally zero; operator, covariate, and model halves remain in Stages 5, 8, 14, and 16 |
...
- [x] Stage 4: Benchmark, birth–death, QCEW-revision, and sample tables\
      ...
      Consumes: From Stage 3: the release-date index for Employment Situation publications and final-benchmark/reconstruction alignment; preliminary benchmark announcements are separate Stage 4 publications. From Stage 2: the data inventory (birth–death and sample rows), the benchmark ruling, the 2025 program-cuts ruling.\
      Produces: the benchmark table (preliminary 2000+ and final 1979+ March revisions with Table 5's footnotes, sector contributions 2003+, a publication date per figure); the reconstruction-events table (the single source of "documented reconstruction"); the birth–death table (monthly forecasts by supersector, revised values, forecast-vs-realized rows; zero for government); the QCEW revision and precision tables (2017+ aggregate sequence in jobs and a revision-precision proxy per benchmark year in thousands); the sample panel (Table 1's coverage share, the Req 3 proxy for usable linked coverage, and RSE by supersector-year, with a definition note where the count of published series falls); every value with provenance and an `observable_at` date.\
      Exit: a final row exists for every benchmark year 1979–2025 and a preliminary row for 2000–2026 (the March 2026 final is due in February 2027), each with a publication date; the March 2025 final row records the article's −862 beside Table 5's −861 as an unexplained difference, and footnote 12's changes are rows of the reconstruction-events table (Stage 2's benchmark ruling); every final-benchmark and reconstruction publication made in an Employment Situation release matches Stage 3's release-date index, while every preliminary benchmark announcement is dated in Stage 4's own table; the birth–death table is zero for government in every row (Req 11, data half); the sample panel labels coverage as the Table 1 proxy, flags the missing March 2012 table, and carries an RSE break flag at each change in the published measure (Req 3; the coverage item in `specs/deferred_items.md`, plan 2); a test asserts every `observable_at` is populated.\
      ROUTING: writing-plans

- [x] Stage 5: Benchmark, aggregation, and seasonal-mapping operators\
...
- [ ] Stage 8: Benchmark anchors, wedge, post-March recursion, and mature horizon in the model\
      ...
      Exit: the fit meets thresholds (bullet 8, comparator (0) NSA); a test asserts reconstruction-error variance is positive in all 23 inferred-year rows, enters each year's post-March likelihood exactly once, and that zeroing it changes the likelihood (bullet 3 closed); a test asserts B rows enter only through the wedge operator with the post-operator residual within rounding for every year (Req 10), and re-derives Stage 3's B→M movement counts by reference-month block (the remaining plan 5 evidence check); right-censored M is integrated out and reported as a posterior predictive, and the preliminary benchmark is an auxiliary observation (Req 2); the T→B stage-mean prior center matches the forecast-vs-realized rows (Req 11); the layer-local prior predictive satisfies the Req 15 wedge and annual-March constraints before the fit; SBC recovers the wedge rank (bullet 7, wedge half); the latent-path-vs-wedge table exists for 2003–2025 (Req 10 output, first version).\
      ROUTING: brainstorming — Stage 5 fixed the cumulative-job-change representation and positive reconstruction-error variance for all 23 public years, but the probabilistic B→M composition and where that variance enters the likelihood remain model-design questions.
...
- [ ] Stage 9: Joint NSA/SA measurement and the model-based seasonal decomposition\
      ...
      Exit: the fit meets thresholds (bullet 8, comparator (0) joint); a test asserts SA rows share the seasonal component with NSA rows and no second seasonal state exists (Req 9); the model's observation mapping reproduces the accounting-decomposition table on the published panel without duplicating or suppressing Stage 8's 23 inferred-year reconstruction-error terms (Req 9); the pre-2003 SA change rows enter through the Req 9 equation (Req 4, model half closed); SBC recovers the sign of the per-stage NSA/factor correlation; the decomposition table exists for comparator (0) (bullet 11, first version).\
      ROUTING: writing-plans
...
- [ ] Stage 17: First fit of comparator (3), sampler decision, and hierarchy geometry\
      ...
      Consumes: From Stage 16: comparator (3), the gated prior table, design tables, harnesses, run harness. From Stage 11: the sampler benchmark record. From Stage 5: the immutable operator-test record and 23-row recoverability table carried through run provenance. From Stage 1: the engine determination and its revisit triggers.\
      ...
      Exit: the fit meets Req 18 thresholds with rank plots for scale, capacity, and hierarchy SDs (bullet 8, comparator (3)); the kernel decision exists with its benchmark evidence, and BlackJAX appears in the pins with a 3.14 import test if and only if adopted (Req 17 step 5); the geometry record names every centered block and a test asserts centered parameterization appears only in blocks it favored (Req 12); run provenance retains the Stage 5 operator-test record and recoverability hash, and all 23 inferred-year reconstruction variances remain active; the two tables exist on comparator (3) (bullet 11, on the reported model). If the benchmark demands a blocked kernel or trips an `engine.md` revisit trigger beyond one plan's scope, the plan's Scope Check splits it out and resume re-validates.\
      ROUTING: writing-plans
...
- [ ] Stage 23: Verified literature review and bibliography\
      ...
      Exit: the four Req 20 parts exist; the review cites the Stage 5 determination and states that all 23 public benchmark years use inferred cumulative job changes with positive reconstruction-error variance; every numeric claim, date, and citation carries a primary-source citation or an unverified flag and an evidence label; the bibliography labels each source's status; `uv run ruff format --check` passes (bullet 13 closed).\
      ROUTING: writing-plans
